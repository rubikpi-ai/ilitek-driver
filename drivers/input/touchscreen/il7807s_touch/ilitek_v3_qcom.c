...
#define DTS_SWAP_X2Y "touch,swapx2y"
...
void ili_input_register(void)
{
	...
	if (ilits->swap_x2y) {
		int X_MAX = ilits->panel_hei;
		int Y_MAX = ilits->panel_wid;
		input_set_abs_params(ilits->input, ABS_MT_POSITION_X, TOUCH_SCREEN_X_MIN, X_MAX - 1, 0, 0);
		input_set_abs_params(ilits->input, ABS_MT_POSITION_Y, TOUCH_SCREEN_Y_MIN, Y_MAX - 1, 0, 0);
	} else {
...
//	ILI_INFO("TP RESET: %d\n", ilits->tp_rst);
	...
	of_property_read_u32(dev_node, DTS_SWAP_X2Y, &ilits->swap_x2y);
